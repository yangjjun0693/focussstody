...
class AppController {
    ...
    async boot() {
        ...
        // Check Supabase session — redirect to login if not authed
        try {
    ...
    async loadAndEnterApp(user) {
        try {
            const [profile, studyRow] = await Promise.all([
                getProfile(user.id),
            ...
            this.pro.isPro = profile.is_pro;
            await this.enterApp(profile.username);
        } catch (err) {
